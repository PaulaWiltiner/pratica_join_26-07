...
SELECT users.id , users.name , courses.name AS course , schools.name AS school , ed."endDate" AS endDate FROM users JOIN educations ed ON users.id = ed."userId" JOIN courses ON ed."courseId" = courses.id JOIN schools ON ed."schoolId" = schools.id WHERE users.id = 30 LIMIT 5;
